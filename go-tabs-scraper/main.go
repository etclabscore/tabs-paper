...
func main() {
	...
	retryLimit := flag.Int("retry-limit", 3, "How many times to re-attempt erroring requests (default=3)")
...
func scrapeBlock(datadir *string, blockN uint64, spanRate *float64, client *ethclient.Client, bkgrnd context.Context, signer types.Signer) error {
	...
	if err != nil {
		return fmt.Errorf("[miner balance] %w", err)
	}
	...
	for txi, tx := range bl.Transactions() {
		...
		if err != nil {
			return fmt.Errorf("[as message] %w", err)
		}
		from := msg.From()
		...
		if err != nil {
			return fmt.Errorf("[tx balance] %w", err)
		}
